...
while a == True:
    ...
    face = max(detections, key=lambda x: x["score"], default=None)
    ...
    face_crop = aligned_face[y1:y2, x1:x2]
    if face_crop.size == 0:
        continue

    last_aligned_crop = face_crop
    ...
    if last_aligned_crop is not None:
        cv2.imshow("Aligned Face", last_aligned_crop)
